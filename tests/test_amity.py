...
class AmityTest(TestCase):
	...
	def setUp(self):
		...
		self.amity.add_person('Yung', 'Ping', fellow='fellow', accomodation=True)
	...
	def test_get_person_details(self):
		...
		for person_key, person_value in persons['staff'].iteritems():
			self.assertEqual(self.amity.get_person_details(person_key).get_first_name(), 'Kim')
			break
	...
	def test_reallocate_staff_to_living_spcae(self):
		persons= self.amity.get_persons_dict()
		for person_key, person_value in persons['staff'].iteritems():
			self.assertFalse(self.amity.reallocate_person(person_key, 'krypton'))
			break

	def test_reallocate_occupant(self):
		persons = self.amity.get_persons_dict()
		for person_key, person_value in persons['fellows'].iteritems():
			self.assertTrue(self.amity.reallocate_person(person_key, 'krypton'))
			break
