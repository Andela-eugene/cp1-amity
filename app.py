...
class MyInteractive(cmd.Cmd):
    ...
    cprint(
        ...
        'magenta')
    cprint(
        "\t\tAmity is a room booking system that allocates free " +
        "space to fellows or staff",
        'yellow')
    cprint(
        '---------------------------------------------------------' +
        '---------------------------------------------------------',
        'magenta')
    cprint(
        "\n\t\t'-h' or '--help' provides full list of " +
    ...
    def do_add_person(self, arg):
        '''Usage: add_person <first_name>
        <last_name> <role> [--accomodation=accom]'''
        allocation_output = None
        if arg['<role>'].lower() == 'staff':
            allocation_output = amity.add_person(
                name=arg['<first_name>'],
                lname=arg['<last_name>'],
                staff=arg['<role>'])

        elif arg['<role>'].lower() == 'fellow':
            if arg['--accomodation'] is None:
                allocation_output = amity.add_person(
                    name=arg['<first_name>'],
                    lname=arg['<last_name>'],
                    fellow=arg['<role>'],
                ...
                allocation_output = amity.add_person(
                    ...
                    fellow=arg['<role>'],
                    accomodation=False)
            elif (arg['--accomodation'].lower() != 'y' or
                  arg['--accomodation'].lower() != 'n'):
                cprint(
                    '                                  ' +
                    'Invalid accommodation parameter',
                    'yellow')
        ...
        cprint(
            ...
            '=================================',
            'magenta')
        cprint('                                  ADD USER {} {}'.format(
            arg['<first_name>'].upper(), arg['<last_name>'].upper()), 'yellow')
        cprint(
            '===========================================' +
            '=================================',
            'magenta')
        for output_key, output_value in allocation_output.iteritems():
            if output_key == 'Office_name':
                if output_value is not None:
                    ...
                    cprint(
                        '                                  ' +
                        'NO OFFICE ALLOCATED, OUT OF SPACE',
                        'yellow')
                    cprint(
                        '-----------------------------------' +
                        '-----------------------------------------',
                        'magenta')
            if output_key == 'Accom_name' and arg['<role>'].lower(
            ) == 'fellow':
                if output_value is not None:
                    cprint('                                  ' +
                           'LIVINGSPACE ALLOCATED: {}'.format(
                               output_value), 'yellow')
                    cprint(
                        '-----------------------------------' +
                        '-----------------------------------------',
                    ...
                    cprint(
                        '                                  ' +
                        'NO LIVINGSPACE ALLOCATED, OUT OF SPACE',
                        'yellow')
                    cprint(
                        '-----------------------------------' +
                        '-----------------------------------------',
    ...
    def do_load_people(self, arg):
        ...
        for load_output in load_return:
            ...
            for output_key, output_value in load_output.iteritems():
                if output_key == 'first_name':
                    ...
                    if wanted_accom:
                        if output_value is not None:
                            ...
                            cprint(
                                '                                  ' +
                                'NO OFFICE ALLOCATED, SPACE UNAVAILABLE',
                                'yellow')
                ...
                elif output_key == 'Accom_name' and wanted_accom:
                    if output_value is not None:
                        cprint('                                  ' +
                               'LIVINGSPACE ALLOCATED: {}'.format(
                                   output_value), 'yellow')
                        ...
                        cprint(
                            '                                  ' +
                            'NO LIVINGSPACE ALLOCATED, SPACE UNAVAILABLE',
                            'yellow')
                        cprint(
                            '-----------------------------------' +
                            '-----------------------------------------',
    ...
    def do_reallocate_person(self, arg):
        ...
        cprint(
            ...
            'magenta')
        cprint('                                  ' +
               'REALLOCATE USER', 'yellow')
        cprint(
            '=========================================' +
            '===================================',
            'magenta')
        is_assigned = amity.reallocate_person(
            arg['<person_identifier>'], arg['<new_room_name>'])
        if is_assigned:
            cprint('                              ' +
                   'SUCCEESFULY REASIGNED', 'green')
            cprint(
                '----------------------------------------' +
                '------------------------------------',
                'magenta')
    ...
    def do_print_allocations(self, arg):
        ...
        cprint(
            '                                 ' +
            'ALLOCATED PERSONS WITH RESPECTIVE ROOMS',
            'yellow')
        cprint(
            '============================================' +
            '================================',
